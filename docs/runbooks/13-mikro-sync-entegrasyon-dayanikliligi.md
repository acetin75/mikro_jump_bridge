...
`mikro_sync` köprüsü Mikro ERP API (port 8094) ile iletişim kurarken ağ/ERP/parser hatalarına karşı **öngörülebilir** ve **veri kaybetmeden** davranmalı.

> **Bağlantılı runbook'lar:** 03 (transaction), 08 (gözlemlenebilirlik).
...
- `sync_motor/client.py` → `fatura_aktar()` farklı hata durumlarında farklı JSON şekilleri döndürebiliyor:
  - `{"hata": "..."}`
...
- `mikro_sync` tarafında `MikroApiClient` ağ hatasında otomatik yeniden deneme yapmıyor (varsayılan davranış).
...
- `mikro_sync` istemcisi farklı bir Mikro ERP sürümüne karşı çalıştığında bunu anlayamaz.
...
1. **Yanıt sözleşmesini sabitle:** `sync_motor/client.py` için ortak `api_yanit(ok, code, message, data, status)` yardımcısı yaz.
