...
| `muhasebe_buro/api_views.py` | `fatura_aktar()` — fatura + kalemler `transaction.atomic()` eklendi |
...
### 2) API üzerinden fatura aktarımı transaction koruması olmadan yazıyor

**Kanıt:**

- `muhasebe_buro/api_views.py` → `fatura_aktar()` içinde önce `Fatura`, sonra döngü ile `FaturaKalemi` oluşturuluyor.
- Akışta transaction yok.

**Risk:**

- Satırların bir kısmı yazılırken hata oluşursa tutarsız veri kalabilir.
